...
module.exports.moviesController = {
  ...
  create: async (req, res) => {
    ...
    if (!movieTitle || !year || !image) {
      throw new CustomError(
        "All fields are mandatory: movieTitle, year, image!",
        400
      );
    }
    ...
    if (!req?.user) {
      throw new CustomError(
        "user_id couldn't be found! -  You should login!",
        401
      );
    }
  ...
  update: async (req, res) => {
    ...
    if( movie?.user_id.toString() !== req?.user?._id.toString()){
        throw new CustomError('Unauthorized - user don\'t have permisson to update this movie!!',401)
    }
    req.body.user_id = req?.user?._id;
  ...
  delete: async (req, res) => {
    ...
    if (deletedMovie) {
      throw new CustomError(
        "Deleted movie still exists! Something went wrong!",
        500
      );
    }
